...
$(document).ready(function() {
	...
	$('#location_tb').DataTable({
		...
		//กำหนดข้อความส่วนหัว --------------------------------------------------
		$("div.toolbar").html('<p style="text-align:center;"><b>สถานที่จัดกิจกรรม (locree Education) </b></p>');
		//กำหนดส่วน footer ให้มีช่องพิมพ์ textbox สำหรับค้นหา
		$('#location_tb tfoot th').each( function () {
			var title = $(this).text();
		...
		$('#location_tb').DataTable().columns().every( function () {
			var that = this;
			//ค้นหาจาก footer
...
<div class="row">
<div class="col-md-12">
             <table id="location_tb" class="display cell-border compact row-border table table-striped table-bordered" cellspacing="0" width="100%">
                            	<thead>
                                      <tr>
                                  ...
                                  <tbody>
                  <?php
                      $sql = " SELECT * FROM tbllocation ORDER BY loc_id DESC";
                      $results = selectSql($sql);
                          $rowint = 1;
                          foreach ($results as $row){
                            $locid = $row['loc_id'];
                            $locname = $row['loc_name'];
                            $locstatus = $row['loc_status'];
									?>
                                  	...
                                  	<tr>
                                        ...
                                        <img src="images/b_edit.png" width="16" height="16" style="cursor:pointer" onClick="javascript:editlocation('<?=$locid?>');">
																				</td>
																					<?php// } ?>
																					<?php// if (!isset($_SESSION["ssUser_id"])) {  echo ""; }else { ?>
                                        <td style="text-align:center;" >
                                        <img src="images/b_drop.png" width="16" height="16" style="cursor:pointer" onClick="javascript:dellocation('<?=$locid?>','<?=$locname?>');">
																				</td>
																					<?php// } ?>
                                    </tr>
