...
$(document).ready(function() {
	...
	$('#bran_tb').DataTable({
		...
		//กำหนดข้อความส่วนหัว --------------------------------------------------
		$("div.toolbar").html('<p style="text-align:center;"><b>สาขาวิชา (ฺBranch) </b></p>');
		//กำหนดส่วน footer ให้มีช่องพิมพ์ textbox สำหรับค้นหา
		$('#bran_tb tfoot th').each( function () {
			var title = $(this).text();
		...
		$('#bran_tb').DataTable().columns().every( function () {
			var that = this;
			//ค้นหาจาก footer
...
<div class="row">
<div class="col-md-12">
             <table id="bran_tb" class="display cell-border compact row-border table table-striped table-bordered" cellspacing="0" width="100%">
                            	<thead>
                                      <tr>
                                  ...
                                  <tbody>
                  <?php
                      $sql = " SELECT * FROM tblbranch, tblcourse WHERE cour_id = bran_courid ORDER BY bran_id DESC";
                      $results = selectSql($sql);
                          $rowint = 1;
                          foreach ($results as $row){
                            $branid = $row['bran_id'];
                            $branname = $row['bran_name'];
														$brannum = $row['bran_num'];
														$courname = $row['cour_name'];
                            $branstatus = $row['cour_status'];
									?>
                                  	...
                                  	<tr>
                                        ...
                                        <img src="images/b_edit.png" width="16" height="16" style="cursor:pointer" onClick="javascript:editbranch('<?=$branid?>');">
																				</td>
																					<?php// } ?>
																					<?php// if (!isset($_SESSION["ssUser_id"])) {  echo ""; }else { ?>
                                        <td style="text-align:center;" >
                                        <img src="images/b_drop.png" width="16" height="16" style="cursor:pointer" onClick="javascript:delbranch('<?=$branid?>','<?=$branname?>');">
																				</td>
																					<?php// } ?>
                                    </tr>
