...
$(document).ready(function() {
	...
	$('#course_tb').DataTable({
		...
		//กำหนดข้อความส่วนหัว --------------------------------------------------
		$("div.toolbar").html('<p style="text-align:center;"><b>หลักสูตรการศึกษา (Course Education) </b></p>');
		//กำหนดส่วน footer ให้มีช่องพิมพ์ textbox สำหรับค้นหา
		$('#course_tb tfoot th').each( function () {
			var title = $(this).text();
		...
		$('#course_tb').DataTable().columns().every( function () {
			var that = this;
			//ค้นหาจาก footer
...
<div class="row">
<div class="col-md-12">
             <table id="course_tb" class="display cell-border compact row-border table table-striped table-bordered" cellspacing="0" width="100%">
                            	<thead>
                                      <tr>
                                        <th style="text-align:center;">ลำดับที่</th>
																				<th>ID</th>
																				<th>หลักสูตรการศึกษา</th>
                                  ...
                                  <tbody>
                  <?php
                      $sql = " SELECT * FROM tblcourse ORDER BY cour_id DESC";
                      $results = selectSql($sql);
                          $rowint = 1;
                          foreach ($results as $row){
                            $courid = $row['cour_id'];
                            $courname = $row['cour_name'];
                            $courstatus = $row['cour_status'];
									?>
                                  	...
                                  	<tr>
                                        ...
                                        <img src="images/b_edit.png" width="16" height="16" style="cursor:pointer" onClick="javascript:editcourse('<?=$courid?>');">
																				</td>
																					<?php// } ?>
																					<?php// if (!isset($_SESSION["ssUser_id"])) {  echo ""; }else { ?>
                                        <td style="text-align:center;" >
                                        <img src="images/b_drop.png" width="16" height="16" style="cursor:pointer" onClick="javascript:delcourse('<?=$courid?>','<?=$courname?>');">
																				</td>
																					<?php// } ?>
                                    </tr>
