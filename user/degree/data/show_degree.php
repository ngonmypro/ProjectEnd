...
$(document).ready(function() {
	...
	$('#type_edu_tb').DataTable({
		...
		//กำหนดข้อความส่วนหัว --------------------------------------------------
		$("div.toolbar").html('<p style="text-align:center;"><b>ระดับการศึกษา (Degree Education) </b></p>');
...
<div class="col-md-12">
             <table id="type_edu_tb" class="display cell-border compact row-border table table-striped table-bordered" cellspacing="0" width="100%">
                            	<thead>
                                  ...
                                  <tbody>
                  <?php
                      $sql = " SELECT * FROM tbldegree ORDER BY deg_id DESC";
                      $results = selectSql($sql);
                          $rowint = 1;
                          foreach ($results as $row){
                            $degid = $row['deg_id'];
                            $degname = $row['deg_name'];
                            $degstatus = $row['deg_status'];
									?>
                                  	...
                                  	<tr>
                                        ...
                                        <img src="images/b_edit.png" width="16" height="16" style="cursor:pointer" onClick="javascript:editdegree('<?=$degid?>');">
																				</td>
																					<?php// } ?>
																					<?php// if (!isset($_SESSION["ssUser_id"])) {  echo ""; }else { ?>
                                        <td style="text-align:center;" >
                                        <img src="images/b_drop.png" width="16" height="16" style="cursor:pointer" onClick="javascript:deldegree('<?=$degid?>','<?=$degname?>');">
																				</td>
																					<?php// } ?>
                                    </tr>
