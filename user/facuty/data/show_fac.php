...
$(document).ready(function() {
	...
	$('#fac_tb').DataTable({
		...
		//กำหนดข้อความส่วนหัว --------------------------------------------------
		$("div.toolbar").html('<p style="text-align:center;"><b>คณะ (Facuty) </b></p>');
		//กำหนดส่วน footer ให้มีช่องพิมพ์ textbox สำหรับค้นหา
		$('#fac_tb tfoot th').each( function () {
			var title = $(this).text();
		...
		$('#fac_tb').DataTable().columns().every( function () {
			var that = this;
			//ค้นหาจาก footer
...
<div class="row">
<div class="col-md-12">
             <table id="fac_tb" class="display cell-border compact row-border table table-striped table-bordered" cellspacing="0" width="100%">
                            	<thead>
                                      <tr>
                                  ...
                                  <tbody>
                  <?php
                      $sql = " SELECT * FROM tblfaculty ORDER BY fac_id DESC";
                            ...
                            $facstatus = $row['fac_status'];
									?>
                                  	...
                                  	<tr>
                                        ...
                                        <img src="images/b_edit.png" width="16" height="16" style="cursor:pointer" onClick="javascript:editfacuty('<?=$facid?>');">
																				</td>
																					<?php// } ?>
																					<?php// if (!isset($_SESSION["ssUser_id"])) {  echo ""; }else { ?>
                                        <td style="text-align:center;" >
                                        <img src="images/b_drop.png" width="16" height="16" style="cursor:pointer" onClick="javascript:delfacuty('<?=$facid?>','<?=$facname?>');">
																				</td>
																					<?php// } ?>
                                    </tr>
